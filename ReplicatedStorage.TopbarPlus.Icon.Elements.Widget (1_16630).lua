-- Name: Widget
-- Path: game:GetService("ReplicatedStorage").TopbarPlus.Icon.Elements.Widget
-- Class: ModuleScript
-- Exploit: Xeno 
-- Time to decompile: 0.6958379999996396 seconds
...
-- Decompiled on 2026-04-10 16:50:29
-- Luau version 6, Types version 3
-- Time taken: 0.027165 seconds
...
return function(arg1, arg2) -- Line 6
		...
		task.defer(function() -- Line 202
			...
			local function _(arg1_4) -- Line 232, Named "getItemWidth"
				...
				if not TargetWidth then
					TargetWidth = arg1_4.AbsoluteSize
					TargetWidth = TargetWidth.X
				end
				return TargetWidth
			end
			local Offset = UIListLayout_upvr.Padding.Offset
			var53 = Offset
			...
			if isSelected then
				for i_2, v_2 in pairs(var10_upvr:GetChildren()) do
					if v_2 ~= Frame_upvr_5 and v_2:IsA("GuiObject") and v_2.Visible then
						...
						if not TargetWidth_2 then
							TargetWidth_2 = v_2.AbsoluteSize
							TargetWidth_2 = TargetWidth_2.X
						end
				...
				if not Frame_upvr_5.Visible then
					...
					if not v_2 then
						v_2 = i_2.AbsoluteSize
						v_2 = v_2.X
					end
					v_2 = MenuUIListLayout_upvr
					v_2 = v_2.Padding
					v_2 = v_2.Offset
					i_2 = v_2 * 2
				end
				var63 -= BorderSize * 0.5
			end
			if isSelected then
			end
			MenuGap_upvr.Visible = Frame_upvr_5.Visible
			local DesiredWidth = Frame_upvr_8:GetAttribute("DesiredWidth")
			if DesiredWidth and ((math.clamp(var53, MinimumWidth, var23_upvr)) - (v_2 + i_2 + BorderSize)) + (var63 - BorderSize * 0.75) < DesiredWidth then
				local var69 = DesiredWidth
			end
			arg1.updateMenu:Fire()
			local var70 = math.max(var69 - var63, MinimumWidth) - BorderSize * 2
			...
			local TargetWidth_4 = Frame_upvr_5:GetAttribute("TargetWidth")
			if not TargetWidth_4 then
				TargetWidth_4 = Frame_upvr_5.AbsoluteSize
				TargetWidth_4 = TargetWidth_4.X
			end
			local TargetWidth_3 = Frame_upvr_8:GetAttribute("TargetWidth")
			if not TargetWidth_3 then
				TargetWidth_3 = Frame_upvr_8.AbsoluteSize
				TargetWidth_3 = TargetWidth_3.X
			end
			...
			TweenService_upvr:Create(Frame_upvr_5, TweenInfo_new_result1, {
				...
				Size = UDim2.new(0, var70, 1, -BorderSize * 2);
			}):Play()
	...
	local function updateBorderSize() -- Line 360
		...
		task.defer(function() -- Line 361
			...
			local var97 = "BorderSize"
			local attribute = Frame_upvr_8:GetAttribute(var97)
			if Frame_upvr_5.Visible == false then
				...
				if arg1.alignment ~= "Right" or not INLINED() then
					var97 = attribute
				end
			end
			var10_upvr.Position = UDim2.new(0, var97, 0, 0)
			MenuGap_upvr.Size = UDim2.fromOffset(attribute, 0)
	...
	arg1.alignmentChanged:Connect(function(arg1_7) -- Line 389
		...
		task.defer(function() -- Line 361
			...
			local var103 = "BorderSize"
			local attribute_2 = Frame_upvr_8:GetAttribute(var103)
			if Frame_upvr_5.Visible == false then
				var103 = 0
			else
				...
				if arg1.alignment ~= "Right" or not INLINED_3() then
					var103 = attribute_2
				end
			end
			var10_upvr.Position = UDim2.new(0, var103, 0, 0)
			MenuGap_upvr.Size = UDim2.fromOffset(attribute_2, 0)
			MenuUIListLayout_upvr.Padding = UDim.new(0, 0)
			any_createStagger_result1_upvr()
		end)
	end)
